...
public class CargadorCarta {
    ...
    private static Image crearImagenDeRespaldo() {
        Canvas lienzo = new Canvas(100, 140);
        GraphicsContext g = lienzo.getGraphicsContext2D();
        g.setFill(Color.WHITESMOKE);
        g.fillRoundRect(0, 0, 100, 140, 16, 16);
        g.setStroke(Color.GRAY);
        g.setLineWidth(2);
        g.strokeRoundRect(1, 1, 98, 138, 16, 16);
        g.setFill(Color.DARKGRAY);
        g.fillText("?", 48, 75);
        SnapshotParameters sp = new SnapshotParameters();
        sp.setFill(Color.TRANSPARENT);
        WritableImage wi = new WritableImage(100, 140);
        lienzo.snapshot(sp, wi);
        return wi;
    }
}
